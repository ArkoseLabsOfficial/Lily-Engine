...
class SaveLoadSlotEntry extends FlxSpriteGroup {
	...
	public function new(x:Float, y:Float, info:SaveManager.SaveSlotData) {
		...
		if (info.isEmpty) {
			...
			bg.loadGraphic(Assets.getImage("ui/saves/save_slot"));

			decor = new FlxSprite(12, 12);
			decor.loadGraphic(Assets.getImage("ui/decors/save_decor"));
			add(decor);
			...
			for (i in 0...info.party.length) {
				...
				portrait.loadGraphic(Assets.getImage(pPath));
	...
	public function select() {
		...
		bg.loadGraphic(Assets.getImage(tex));
		bg.setGraphicSize(SLOT_W, SLOT_H);
		bg.updateHitbox();
