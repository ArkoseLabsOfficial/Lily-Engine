...
class Settings extends SubStateBackend {
	...
	function parseXML(menuId:String):Void {
		var xmlString = LilyAssets.getTextFromFile(Flags.settingsFile);
...
class SettingsMenu extends SimpleVerticalMenu {
	public var options:Array<XmlOption> = [];
	public var uiScale:Float = 1.0;
	public var optionGap:Float = 72;
	public var currentMenuId:String = "main";
	public var isListening:Bool = false;
	public var parentState:Settings;
	...
	override public function acceptSelection():Void {
		...
		if (currentMenuId == "main") {
			...
			if (opt.target == "language")
				parentState.openSubState(new Language());
			else
				parentState.openSubState(new Settings(opt.target, parentState.fromPause));
		} else {
			if (opt.type == "bool") {
				LilyAssets.play(LilyAssets.CONFIRM);
