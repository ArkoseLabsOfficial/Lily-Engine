...
class LacieShader extends FlxShader implements IHScriptCustomBehaviour {
	private static var __instanceFields = Type.getInstanceFields(LacieShader);
	...
	public var fileName:String = "LacieShader";
	public var fragFileName:String = "LacieShader";
	public var vertFileName:String = "LacieShader";
	...
	public override function new(frag:String, vert:String, glslVer:String = null) {
		if (glslVer == null) glslVer = #if mobile "100" #else "120" #end;
	...
	private function processImports(value:String, type:Int):String
	{
		while(IMPORT_REGEX.match(value))
		{
			...
			if(importSource == null) {
                ...
                trace("Failed to import shader ${importPath} in ${fileName}");
			} else {
	...
	@:noCompletion private override function __createGLProgram(vertexSource:String, fragmentSource:String):GLProgram
	{
		...
		{
            trace('Failed to compile shader ${fileName}: ' + Std.string(error));
		}
		return program;
