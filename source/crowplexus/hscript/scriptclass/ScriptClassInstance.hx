...
class ScriptClassInstance extends BaseScriptClass {
	...
	var constructorThing: BydFieldDecl;
	var urDad: ScriptClass;

	private function new(ogInterp: Interp, name: String, extend: String, fields: Array<BydFieldDecl>, constructorThing: BydFieldDecl, parent: ScriptClass,
			?constructorArgs: Array<Dynamic>) {
		this.name = name;
		this.extend = extend;
		this.fields = fields;
		this.constructorThing = constructorThing;
		this.urDad = parent;
	...
	function parseConstructor() {
		@:privateAccess if (this.constructorThing != null) {
			final field = this.constructorThing;
			switch (field.kind) {
				case KFunction(decl):
					__interp.directorFields.set(field.name, {
						value: parseFuncDecl(decl, field.name),
			...
			throw "ScriptClass '" + name + "' has not constructorThing.";
		}
	}
