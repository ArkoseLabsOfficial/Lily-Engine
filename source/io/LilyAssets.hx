...
class LilyAssets {
	...
	public static final dumpExclusions:Array<String> = ['images/ui/cursor.$IMAGE_EXT', 'images/ui/cursorCross.$IMAGE_EXT'];
	...
	inline public static function music(key:String):Sound
	...
	public static function returnSound(path:Null<String>, key:String):Sound {
		for (ext in SOUND_EXTS) {
			...
			if (path != null)
				gottenPath = (path != '' && path != null) ? '$path/' + gottenPath : gottenPath;
			...
			if (!currentTrackedSounds.exists(gottenPath)) {
				if (FileSystem.exists(resolvedPath))
					currentTrackedSounds.set(gottenPath,
						#if html5 openfl.Assets.getSound(resolvedPath) #else Sound.fromBytes(File.getBytes(resolvedPath)) #end);
			}
